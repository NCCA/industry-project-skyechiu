...
def process_single_image(
    ...
    ply_path = os.path.join(demo_output_local_dir, "gaussians.ply")
    if os.path.exists(ply_path):
        print(f"Skipping {image_file} (gaussians.ply exists)")
        return
    ...
    # Generate turntable video (skip if SKIP_TURNTABLE env var is set)
    if not os.environ.get("SKIP_TURNTABLE"):
        ...
        print(f"  Skipping turntable video (SKIP_TURNTABLE=1)")
